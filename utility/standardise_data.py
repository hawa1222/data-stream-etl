...
class CleanData:
    ...
    def drop_na_rows(df, threshold):
        """
        Drop rows with more than 'threshold' NaN values
        """
    ...
    def clean_col_names(col):
        ...
        """
        col = col.strip()  # - Leading/trailing whitespaces
        col = re.sub(r"\s+", "_", col)  # Replace whitespaces with underscores
        col = re.sub("(?<=[a-z])(?=[A-Z])", "_", col)  # + Underscore between camelCase

        return col.lower()

    def clean_data(df, na_threshold):
        """
        Drop NaN rows and standardise column names
        """
        ...
        # Standardise column names
        df.columns = [CleanData.clean_col_names(col) for col in df.columns]
        logger.info("Standardised column names")
