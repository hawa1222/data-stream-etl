...
class FileDirectory:
    ...
    MANUAL_EXPORT_PATH = '/Users/hadid/Library/Mobile Documents/com~apple~CloudDocs/Shared/ETL'
    # Raw Data Folder
...
class AppleHealth:
    ...
    # File Names
    APPLE_XML_PATH = os.path.join(FileDirectory.MANUAL_EXPORT_PATH, 'apple_health_export')
    ...
    RECORD_ELEMENTS = [
        'AppleWalkingSteadiness', 'BasalEnergyBurned', 'BloodGlucose', 'FlightsClimbed',
        'HeartRate', 'HeartRateRecoveryOneMinute', 'HeartRateVariabilitySDNN', 'LowHeartRateEvent',
        'MindfulSession', 'OxygenSaturation', 'RespiratoryRate', 'RestingHeartRate','RunningGroundContactTime', 
        'RunningPower', 'RunningSpeed', 'RunningStrideLength', 'RunningVerticalOscillation', 'SleepAnalysis',
        'StepCount', 'VO2Max', 'WalkingAsymmetryPercentage', 'WalkingDoubleSupportPercentage',
    ...
    PRIORITY_DICT = {'HW3 iWatch': 1, 'HW3 iPhone': 2, 'Clock': 3, 'Sleep Cycle': 4,
                     'Health': 5, 'Dexcom G7': 6, 'Headspace': 7, 'RunGap': 8, 'Blood Oxygen': 9}
    ...
    RECORD_TRANSFORMATION_LOGIC = {
        ...
        "HeartRate": {"timeframe": ["date", "hour"], "agg_type": ["Mean"], "fields": ["avg_HR_rate"], "group": ["heart_rate"]}, 
        "HeartRateRecoveryOneMinute": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["avg_1min_HR_recovery"], "group": ["fitness_metrics"]}, 
        "HeartRateVariabilitySDNN": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["avg_HR_variability"], "group": ["fitness_metrics"]}, 
        "LowHeartRateEvent": {"timeframe": ["date", "hour"], "agg_type": ["Sum"], "fields": ["low_HR_event"], "group": ["low_HR_events"]}, 
        "MindfulSession": {"timeframe": ["date"], "agg_type": ["Sum"], "fields": ["mindful_duration", "mindful_count"], "group": ["daily_activity"]}, 
        "OxygenSaturation": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["avg_oxg_saturation"], "group": ["fitness_metrics"]}, 
        "RespiratoryRate": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["avg_respiratory_pm"], "group": ["fitness_metrics"]}, 
        "RestingHeartRate": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["avg_resting_HR"], "group": ["fitness_metrics"]}, 
        "RunningGroundContactTime": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["avg_run_gct_ms"], "group": ["running_metrics"]}, 
        "RunningPower": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["avg_run_pwr_w"], "group": ["running_metrics"]}, 
        "RunningSpeed": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["avg_run_spd_kmh"], "group": ["running_metrics"]}, 
        "RunningStrideLength": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["run_stride_len_m"], "group": ["running_metrics"]}, 
        "RunningVerticalOscillation": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["run_vert_osc_cm"], "group": ["running_metrics"]}, 
        "SleepAnalysis": {"timeframe": ["date"], "agg_type": ["Sum"], "fields": ["total_time_asleep", "bed_time", "awake_time", "sleep_counts", "rem_cycles"], "group": ["sleep"]}, 
        "StepCount": {"timeframe": ["date", "hour"], "agg_type": ["Sum"], "fields": ["step_count"], "group": ["steps"]}, 
        "VO2Max": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["vo2Max_mLmin_kg"], "group": ["fitness_metrics"]}, 
        "WalkingAsymmetryPercentage": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["walking_asymm_pct"], "group": ["walking_metrics"]}, 
        "WalkingDoubleSupportPercentage": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["walking_ds_pct"], "group": ["walking_metrics"]}, 
        "WalkingHeartRateAverage": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["walking_avg_HR"], "group": ["walking_metrics"]}, 
        "WalkingSpeed": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["walking_speed_kmhr"], "group": ["walking_metrics"]}, 
        "WalkingStepLength": {"timeframe": ["date"], "agg_type": ["Mean"], "fields": ["walking_step_len_cm"], "group": ["walking_metrics"]}
        }
    
    ACTIVITY_TRANSFORMATION_LOGIC = {
        "ActivitySummary": {"timeframe": ["date"], "agg_type": ["Sum"], "fields": ["date_components", "active_energy_burned", "apple_exercise_time", "apple_stand_hours"], "group": ["daily_activity"]}
        }
...
class StravaAPI:
    ...
    # File Names
    SUMMARY_CACHE_FILE = 'strava_summary_cache.xlsx'
...
class Youtube:
    ...
    LIKES_FIELDS = [DATE, 'playlist', 'source', 'video_id', 'title', DESC,
                    THUMBNAIL, 'video_owner_channel_title', 'channel_id']
