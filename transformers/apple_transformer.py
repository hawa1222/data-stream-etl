...
def process_activity_summary(df):
    ...
    columns_to_keep = [AppleHealth.DATE_COMPONENTS, AppleHealth.ACTIVE_ENERGY_BURNED, 
                       AppleHealth.APPLE_EXERCISE_TIME, AppleHealth.APPLE_STAND_HOURS]
...
def process_record(df):
    ...
    def transform_df(df):
        ...
        date_cols = [AppleHealth.CREATION_DATE, AppleHealth.START_DATE, AppleHealth.END_DATE]
        ...
        df[AppleHealth.DATE] = df[AppleHealth.CREATION_DATE].dt.strftime(Settings.DATE_FORMAT)
        ...
        df[AppleHealth.VALUE] = pd.to_numeric(df[AppleHealth.VALUE], errors='coerce').fillna(1.0)
...
def subset_by_priority(df):
    ...
    df[AppleHealth.PRIORITY] = df[AppleHealth.SOURCE_NAME].map(AppleHealth.PRIORITY_DICT)
    ...
    df = df[df[AppleHealth.PRIORITY] == df.groupby(AppleHealth.DATE)[AppleHealth.PRIORITY].transform('min')]
...
# =============================================================================
    ...
    df[AppleHealth.ORIGINAL_VALUE] = df[AppleHealth.ORIGINAL_VALUE].str.split('SleepAnalysis').str.get(-1)
    ...
    df[AppleHealth.DURATION] = (df[AppleHealth.END_DATE] - df[AppleHealth.START_DATE]).dt.total_seconds() / 3600
    
    # Group by 'date' and 'original_value', then sum the 'duration' for each group
    pivot_df = df.pivot_table(index=AppleHealth.DATE, columns=AppleHealth.ORIGINAL_VALUE, values=AppleHealth.DURATION, aggfunc='sum').reset_index()
    
    # Add 'bed_time', 'awake_time', and 'source_name' columns to pivot_df
    grouped = df.groupby(AppleHealth.DATE)
    pivot_df[AppleHealth.BED_TIME] = grouped[AppleHealth.START_DATE].min().reset_index()[AppleHealth.START_DATE]
    pivot_df[AppleHealth.AWAKE_TIME] = grouped[AppleHealth.END_DATE].max().reset_index()[AppleHealth.END_DATE]
    pivot_df[AppleHealth.SOURCE_NAME] = grouped[AppleHealth.SOURCE_NAME].first().reset_index()[AppleHealth.SOURCE_NAME]
    pivot_df[AppleHealth.TIME_IN_BED] = (pivot_df[AppleHealth.AWAKE_TIME] - pivot_df[AppleHealth.BED_TIME]).dt.total_seconds() / 3600
    ...
    condition = pivot_df['Awake'].isna() & (pivot_df['InBed'] != pivot_df[AppleHealth.TIME_IN_BED])
    pivot_df.loc[condition, 'Awake'] = pivot_df[AppleHealth.TIME_IN_BED] - pivot_df['InBed']
...
def transform_record_dicts(record_dict: Dict[str, DataFrame]) -> Dict[str, DataFrame]:
    ...
    for df_name in AppleHealth.RECORD_ELEMENTS:
        ...
        if df_name == 'MindfulSession':
            new_df.loc[:, AppleHealth.DURATION] = (new_df[AppleHealth.END_DATE] - new_df[AppleHealth.START_DATE]).dt.total_seconds() / 60
            agg_dict.update(dict(zip(fields, [(AppleHealth.DURATION, 'sum'), (AppleHealth.VALUE, 'count')])))
            ...
            agg_dict = {field: (AppleHealth.VALUE, agg_type.lower()) for field in fields}
...
def join_data_by_group(complete_dict):
    ...
    merger_logic = {**AppleHealth.RECORD_TRANSFORMATION_LOGIC, **AppleHealth.ACTIVITY_TRANSFORMATION_LOGIC}
...
def apple_transformer():
    ...
    standardise_dates(joined_data_by_group['sleep'], [AppleHealth.BED_TIME, AppleHealth.AWAKE_TIME])
    ...
    doc_path = os.path.join(FileDirectory.DOCUMENTATION_PATH, generate_filename(AppleHealth.DOCUMEMTATION_DATA))
